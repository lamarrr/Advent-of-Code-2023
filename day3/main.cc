...
void get_number_coordinates(std::span<char const> data, std::span<Slice const> lines,
                            std::vector<std::vector<PartNumber>> &numbers)
{
  ...
  for (u32 row = 0; row < (u32) lines.size(); row++)
  {
    ...
    for (usize i = line.offset; i < line.offset + line.size; i++)
    {
      ...
      {
        i32 part_number;
        std::from_chars(data.data() + num_begin, data.data() + num_begin + num_length, part_number);
        numbers[row].push_back(PartNumber{.value       = part_number,
                                          .row         = row,
...
void get_symbol_coordinates(std::span<char const> data, std::span<Slice const> lines,
                            std::vector<std::vector<Symbol>> &symbols)
{
  ...
  for (u32 row = 0; row < (u32) lines.size(); row++)
  {
    ...
    for (usize i = line.offset; i < line.offset + line.size; i++)
    {
      if (is_symbol(data[i]))
      {
        symbols[row].push_back(
            Symbol{.value = data[i], .row = row, .column = (u32) (i - line.offset)});
      }
    }
  }
}
           ...
           (search_row < (row + 2)) && (search_row < num_rows); search_row++)
      {
        for (Symbol const &symbol : symbols[search_row])
        {
          if ((symbol.column + 1) >= part_number.column &&
              symbol.column < (part_number.column + part_number.column_span + 1))
          {
            is_valid = true;
            break;
          }
