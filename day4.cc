...
constexpr bool is_non_num(char c) { return !is_num(c); }
...
int main(int argc, char const **argv)
{
  ...
  for (std::span<char const> card : cards)
  {
    ...
    intersection.clear();
    std::span sets_text =
        advance_to_token(card, [](char c)
                         { return c == ':'; });
    std::span next_section =
        advance_to_token(sets_text, [](char c)
                         { return c == '|'; });
    ...
    u64 curr_points = (intersection.size() == 0) ? 0ULL : (1ULL << (((u64)intersection.size()) - 1ULL));
    points += curr_points;
  }
